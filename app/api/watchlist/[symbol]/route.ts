...
export async function POST(request: NextRequest, { params }: { params: { symbol: string } }) {
  try {
    ...
    // 관심종목 추가
    const watchlistItem = await addToWatchlist(session.user.id, validatedData.symbol, validatedData.company);
    ...
    if (error instanceof Error && error.name === 'ZodError') {
      return NextResponse.json({ success: false, error: 'Invalid symbol or company name' }, { status: 400 });
    }

    // 중복 오류
    if (error && typeof error === 'object' && 'code' in error && error.code === 11000) {
      return NextResponse.json({ success: false, error: 'Stock already in watchlist' }, { status: 409 });
    }

    // 제한 초과 오류
    if (error instanceof Error && error.message?.includes('limit')) {
      return NextResponse.json({ success: false, error: 'Watchlist limit reached (50 stocks)' }, { status: 413 });
    }
...
export async function DELETE(request: NextRequest, { params }: { params: { symbol: string } }) {
  try {
    ...
    if (!result) {
      return NextResponse.json({ success: false, error: 'Stock not found in watchlist' }, { status: 404 });
    }
